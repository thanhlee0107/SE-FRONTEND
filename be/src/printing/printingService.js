...
async function simulatePrintTask(IDPrinter,task) {
    return new Promise((resolve) => {
        setTimeout(() => {
            console.log(`Máy in ${IDPrinter}: Đang in tài liệu "${task.IDFile}" với số lượng ${task.Amount}, kích thước ${task.Size} và màu ${task.Color}`);
            resolve();
...
async function processPrinterQueue(IDPrinter) {
    ...
    while (true) {
        if (printerQueues[IDPrinter].length > 0) {
            ...
            await dbPrinting.updatePrintStatus(task.IDFile, IDPrinter, task.Amount, task.Date, "Completed");
        } else {
...
exports.startPrinters = () => {
    for(i = 0; i < 2; ++i){
        processPrinterQueue(i);
    }
}
...
const addPrinterJob = async (IDUser, req) => {
    try{
        ...
        await dbPrinting.insertPrintStatus(IDFile, IDPrinter, Amount, printDate, "Waiting");

        console.log(`Yêu cầu in "${task}" đã được thêm vào hàng đợi của máy in ${IDPrinter}`);
    } catch(error){
