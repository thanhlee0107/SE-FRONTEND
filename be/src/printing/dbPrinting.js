  ...
  CREATE TABLE IF NOT EXISTS Printing (
    ...
    FOREIGN KEY (IDPrinter) REFERENCES Printer(ID),
  ...
  exports.checkAndInsertPrinting = async (IDFile, IDPrinter, StudentID) => {
    try {
      ...
      console.error("Lỗi khi kiểm tra hoặc thêm dữ liệu vào bảng Printing:", error.stack); // In ra stack trace để debug dễ dàng hơn
      throw error;
    }
  };

  // Hàm thêm công việc in vào bảng PrintStatus
  exports.insertPrintStatus = async (IDFile, IDPrinter, Amount, printDate, Status) => {
    try {
  ...
  exports.updatePrintStatus = async (IDFile, IDPrinter, Amount, printDate, Status) => {
    try {
      ...
      `;
      const result = await query(updateQuery, [Status, IDFile, IDPrinter, Amount, dateStr]);
